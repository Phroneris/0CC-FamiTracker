...
void CPatternEditor::ApplyColorScheme()
{
	...
	m_colEmptyBg	= DIM(FTEnv.GetSettings()->Appearance.iColBackground, SHADE_LEVEL::EMPTY_BG);
...
{
	m_iPatternLength = GetCurrentPatternLength(GetFrame());
	// // //
}
...
//#define BENCHMARK
	...
	if (bDrawPattern) {
		...
		// Todo: fix this
		if (FTEnv.GetSettings()->General.bFreeCursorEdit)
			bQuickRedraw = false;

		// Todo: remove
		m_iDrawCursorRow = GetRow();
...
CRect CPatternEditor::GetInvalidatedRect() const
{
	...
	else if (FTEnv.GetSettings()->Display.bRegisterState)		// // //
		return GetActiveRect();
...
bool CPatternEditor::CursorUpdated()
{
	...
	if (m_iFirstChannel > GetChannel()) {
		m_iFirstChannel = GetChannel();
		bUpdateNeeded = true;
	}

	// Channel cursor moved to the right of all visible channels
	while ((GetChannel() - m_iFirstChannel) > (m_iChannelsFullVisible - 1)) {
		++m_iFirstChannel;
		bUpdateNeeded = true;
	}
	...
	for (int i = 0; i < ChannelCount; ++i) {
		if (i == GetChannel()) {
			if (GetColumn() > m_iColumns[i])
				m_cpCursorPos.Xpos.Column = m_iColumns[i];
		}
	}

	if (GetRow() > m_iPatternLength - 1)
	...
	if (FTEnv.GetSettings()->General.bFreeCursorEdit) {
		...
		int CursorDifference = GetRow() - m_iCenterRow;
		...
		while (CursorDifference >= (m_iLinesFullVisible / 2) && CursorDifference > 0) {
			...
			m_iCenterRow += 1;
			CursorDifference = (GetRow() - m_iCenterRow);
		}
...
void CPatternEditor::PerformQuickRedraw(CDC &DC)
{
	...
	else if (!FTEnv.GetSoundGenerator()->IsPlaying() && m_iLastPlayRow != -1) {
		if (m_iPlayFrame == GetFrame()) {
			int Line = RowToLine(m_iLastPlayRow);
			if (Line >= 0 && Line <= m_iLinesVisible) {
				// Erase
				PrintRow(DC, m_iLastPlayRow, Line, GetFrame());
			}
		}
	}
...
{
	const int FrameCount = GetFrameCount();		// // //
	const int rEnd = (FTEnv.GetSoundGenerator()->IsPlaying() && m_bFollowMode) ? std::max(m_iPlayRow + 1, m_iPatternLength) : m_iPatternLength;
	if (Row >= 0 && Row < rEnd) {
		DrawRow(DC, Row, Line, Frame, false);
	}
	else if (FTEnv.GetSettings()->General.bFramePreview) {
...
void CPatternEditor::DrawRow(CDC &DC, int Row, int Line, int Frame, bool bPreview) const
{
	...
	if (Frame != GetFrame() && !pSettings->General.bFramePreview) {
		ClearRow(DC, Line);
		return;
	}
	...
	const double BlendLv = Frame == GetFrame() ? 1. : PREVIEW_SHADE_LEVEL;
	...
	for (int i = m_iFirstChannel; i < m_iFirstChannel + m_iChannelsVisible; ++i) {
		...
		if (!m_bFollowMode && Row == m_iPlayRow && f == m_iPlayFrame && FTEnv.GetSoundGenerator()->IsPlaying()) {
			// Play row
		...
		for (unsigned j_ = 0; j_ <= value_cast(Columns); ++j_) {
			...
			// Draw cursor box
			if (i == GetChannel() && j == GetColumn() && Row == m_iDrawCursorRow && !bPreview) {
...
void CPatternEditor::DrawRegisters(CDC &DC) {		// // //
	...
	if (FTEnv.GetSettings()->Display.bRegisterState)		// // //
...
void CPatternEditor::UpdateVerticalScroll()
{
	...
	si.nMin = 0;
	si.nMax = m_iPatternLength + FTEnv.GetSettings()->General.iPageStepSize - 2;
	si.nPos = m_iDrawCursorRow;
	si.nPage = FTEnv.GetSettings()->General.iPageStepSize;
...
void CPatternEditor::UpdateHorizontalScroll()
{
	...
	for (int i = 0; i < Channels; ++i) {
		if (i == GetChannel())
			CurrentColumn = ColumnCount + value_cast(GetColumn());
...
CCursorPos CPatternEditor::GetCursorAtPoint(const CPoint &point) const
{
	...
	if (FTEnv.GetSettings()->General.bFramePreview) {		// // // guarantees valid cursor position
		while (Row < 0) {
			Row += GetCurrentPatternLength(--Frame);
		}
		while (Row >= GetCurrentPatternLength(Frame)) {
...
CPatternEditor::CSelectionGuard::~CSelectionGuard()		// // //
{
	...
	if (IsShiftPressed()) {
		...
		if (FTEnv.GetSettings()->General.iEditStyle != edit_style_t::IT || !m_pPatternEditor->m_bSelecting)
...
void CPatternEditor::MoveChannelLeft()
{
	...
	cursor_column_t Columns = GetChannelColumns(GetChannel());
...
void CPatternEditor::OnHomeKey()
{
	...
	if (bControl || FTEnv.GetSettings()->General.iEditStyle == edit_style_t::FT2) {
...
void CPatternEditor::MoveToRow(int Row)
{
	...
	if (FTEnv.GetSettings()->General.bWrapFrames) {		// // //
		while (Row < 0) {
	...
	else if (FTEnv.GetSettings()->General.bWrapCursor) {
		Row %= m_iPatternLength;
...
void CPatternEditor::MoveToFrame(int Frame)
{
	...
	if (FTEnv.GetSettings()->General.bWrapFrames) {
		if (m_bSelecting) {		// // //
			if (FTEnv.GetSettings()->General.bMultiFrameSel) {		// // //
				if (Frame < 0)
					--m_iWarpCount;
				else if (Frame / FrameCount > GetFrame() / FrameCount)
					++m_iWarpCount;
			}
		}
		Frame %= FrameCount;
	...
	if (m_bSelecting && !FTEnv.GetSettings()->General.bMultiFrameSel)		// // //
		m_selection.m_cpStart.Ypos.Frame = m_selection.m_cpEnd.Ypos.Frame = Frame;

	if (FTEnv.GetSoundGenerator()->IsPlaying() && m_bFollowMode) {
		if (m_iPlayFrame != Frame) {
			FTEnv.GetSoundGenerator()->MoveToFrame(Frame);
...
void CPatternEditor::MoveToChannel(int Channel)
{
	...
	if (Channel < 0) {
		if (FTEnv.GetSettings()->General.bWrapCursor)
...
{
	if (GetColumn() > cursor_column_t::NOTE)
		m_cpCursorPos.Xpos.Column = static_cast<cursor_column_t>(value_cast(GetColumn()) - 1);
	else {
		if (GetChannel() > 0) {
			--m_cpCursorPos.Xpos.Track;
			m_cpCursorPos.Xpos.Column = m_iColumns[GetChannel()];
		}
		else {
			if (FTEnv.GetSettings()->General.bWrapCursor) {
...
{
	if (GetColumn() < m_iColumns[GetChannel()])
		m_cpCursorPos.Xpos.Column = static_cast<cursor_column_t>(value_cast(GetColumn()) + 1);
	else {
		if (GetChannel() < GetChannelCount() - 1) {
			++m_cpCursorPos.Xpos.Track;
			m_cpCursorPos.Xpos.Column = cursor_column_t::NOTE;
		}
		else {
	...
	if (m_bDragStart) {
		...
		if (!FTEnv.GetSettings()->General.bMultiFrameSel) {		// // //
			...
			if (m_selection.m_cpEnd.Ypos.Frame > m_selection.m_cpStart.Ypos.Frame) {
				...
				m_selection.m_cpEnd.Ypos.Row = GetCurrentPatternLength(GetFrame()) - 1;
			}
...
void CPatternEditor::OnMouseScroll(int Delta)
{
	...
	if (Delta != 0) {
		int ScrollLength = (Delta < 0) ? FTEnv.GetSettings()->General.iPageStepSize : -FTEnv.GetSettings()->General.iPageStepSize;
		m_cpCursorPos.Ypos.Row += ScrollLength;

		if (FTEnv.GetSettings()->General.bWrapFrames) {		// // //
			while (GetRow() < 0) {
				if (GetFrame() == 0 && m_bSelecting) ++m_iDragBeginWarp;
				MoveToFrame(GetFrame() - 1);
				m_cpCursorPos.Ypos.Row += m_iPatternLength;
			}
			while (GetRow() > (m_iPatternLength - 1)) {
				m_cpCursorPos.Ypos.Row -= m_iPatternLength;
				MoveToFrame(GetFrame() + 1);
				if (GetFrame() == 0 && m_bSelecting) --m_iDragBeginWarp;
			}
		...
		m_iCenterRow = GetRow();
		if (FTEnv.GetSettings()->General.iEditStyle != edit_style_t::IT && m_bSelecting == false)
			CancelSelection();
	}
}
...
void CPatternEditor::Paste(const CPatternClipData &ClipData, paste_mode_t PasteMode, paste_pos_t PastePos)		// // //
{
	...
	const int f = AtSel ? m_selection.GetFrameStart() : (PastePos == paste_pos_t::DRAG ? m_selDrag.GetFrameStart() : GetFrame());
	const unsigned int r = AtSel ? m_selection.GetRowStart() : (PastePos == paste_pos_t::DRAG ? m_selDrag.GetRowStart() : GetRow());
	const unsigned int c = AtSel ? m_selection.GetChanStart() : (PastePos == paste_pos_t::DRAG ? m_selDrag.GetChanStart() : GetChannel());
	...
	if (Channels == 1 && StartColumn >= column_t::Effect1) {
		const column_t ColStart = std::max(GetSelectColumn(AtSel ? m_selection.GetColStart() : GetColumn()), column_t::Effect1);
		for (unsigned int j = 0; j < Rows; ++j) {
			...
			if ((++it).m_iRow == 0) { // end of frame reached
				if ((!FTEnv.GetSettings()->General.bOverflowPaste) || PasteMode == paste_mode_t::INSERT)
					break;
			}
...
void CPatternEditor::SelectChannel()
{
	...
	m_bSelecting = true;
	SetSelectionStart({0, GetChannel(), cursor_column_t::NOTE, GetFrame()});		// // //
	SetSelectionEnd({m_iPatternLength - 1, GetChannel(), GetChannelColumns(GetChannel()), GetFrame()});
}
...
void CPatternEditor::SelectAllChannels()
{
	// Select all channels
	m_bSelecting = true;
	SetSelectionStart({0, 0, cursor_column_t::NOTE, GetFrame()});		// // //
	SetSelectionEnd({m_iPatternLength - 1, GetChannelCount() - 1, GetChannelColumns(GetChannelCount() - 1), GetFrame()});
}
...
void CPatternEditor::SelectAll()
{
	...
	if (m_bSelecting) {
		if (m_selection.GetChanStart() == GetChannel() && m_selection.GetChanEnd() == GetChannel()) {
			if (m_selection.GetRowStart() == 0 && m_selection.GetRowEnd() == m_iPatternLength - 1) {
				if (m_selection.GetColStart() == cursor_column_t::NOTE && m_selection.GetColEnd() == GetChannelColumns(GetChannel()))
					selectAll = true;
			}
		}
	}
...
sel_condition_t CPatternEditor::GetSelectionCondition(const CSelection &Sel) const
{
	...
	if (!FTEnv.GetSettings()->General.bShowSkippedRows) {
...
void CPatternEditor::DecreaseEffectColumn(int Channel)
{
	...
	if (GetMainFrame()->AddAction(std::make_unique<CPActionEffColumn>(Channel, Columns)))		// // //
		if (static_cast<int>(GetColumn()) - static_cast<int>(cursor_column_t::VOLUME) > Columns * 3)		// // //fx
			m_cpCursorPos.Xpos.Column = static_cast<cursor_column_t>(value_cast(GetColumn()) - 3);
}

bool CPatternEditor::IsPlayCursorVisible() const
{
	if (m_iPlayFrame > (GetFrame() + 1))
		return false;

	if (m_iPlayFrame < (GetFrame() - 1))
		return false;

	if (m_iPlayFrame != (GetFrame() + 1) && m_iPlayFrame != (GetFrame() - 1)) {
...
bool CPatternEditor::ScrollTimerCallback()
{
	...
	if (GetRow() == GetCurrentPatternLength(GetFrame())) {		// // //
	...
	case SCROLL_LEFT:
		if (m_iFirstChannel > 0) {
			--m_iFirstChannel;
			if (GetChannel() >= m_iFirstChannel + m_iChannelsFullVisible)
				--m_cpCursorPos.Xpos.Track;
			InvalidateBackground();
		}
		break;
...
bool CPatternEditor::PerformDrop(CPatternClipData ClipData, bool bCopy, bool bCopyMix)		// // //
{
	...
	if (m_selDrag.m_cpStart.Xpos.Track < 0/* || m_selDrag.m_cpStart.Ypos.Row < 0*/) {
		...
		if (m_selDrag.m_cpStart.Xpos.Track < 0)
			m_selDrag.m_cpStart.Xpos = {0, cursor_column_t::NOTE};
	...
	if (m_selDrag.m_cpEnd.Xpos.Track > Channels - 1)
		m_selDrag.m_cpEnd.Xpos = {Channels - 1, GetChannelColumns(Channels)};
