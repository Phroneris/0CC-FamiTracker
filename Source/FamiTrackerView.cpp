...
const LPCWSTR EFFECT_TEXTS[] = {		// // //	/// jp
	L"(未使用)",
	L"Fxx - 行Tick を XX に設定、グルーヴを取り消し。[※ 1F 以下の時。20 以上では別]",
	L"Fxx - BPM を XX に設定。[※ 20 以上の時。1F 以下では別]",
	L"Bxx - フレーム XX の始端に飛ぶ。",
	L"Dxx - 次のフレームの行 XX に飛ぶ。",
	L"Cxx - 楽曲を停止。",
	L"Exx - 長さカウンタ番号を XX に設定。[※ XX≦1F。EEx でモード設定。README 要参照]",
	L"EEx - 長さカウンタのモード設定。bit 0 = 有効化、bit 1 = カウンタ使用 [※ Exx でカウンタ番号。README 要参照]",
	L"3xx - 自動ポルタメント。XX = 速さ",
	L"(未使用)",
	L"Hxy - 上昇ハードウェアスウィープ。X = 速さ、Y = 幅",
	L"Ixy - 下降ハードウェアスウィープ。X = 速さ、Y = 幅",
	L"0xy - アルペジオ。X = 2 音目、Y = 3 音目 [※ Y = 0 で 2 音だけになる]",
	L"4xy - ビブラート。X = 速さ、Y = 深さ",
	L"7xy - トレモロ。X = 速さ、Y = 深さ",
	L"Pxx - 音程。XX - 80 = オフセット [※ つまり P80 が基準音程]",
	L"Gxx - 行の遅延。XX = フレーム数 [※ 3 連符の作成に便利]",
	L"Zxx - DPCM デルタカウンタの設定。XX = DC バイアス [※ XX≦7F]",
	L"1xx - 音程の上昇スライド。XX = 速さ",
	L"2xx - 音程の下降スライド。XX = 速さ",
	L"Vxx - 矩形波のデューティ比やノイズモードを XX に設定。",
	L"Vxx - N163 の波形番号を XX に設定。",
	L"Vxx - VRC7 のパッチ番号を XX に設定。",
	L"Yxx - DPCM サンプル開始位置のオフセットを XX に設定。[※ 1 ごとに 64 バイトぶん]",
	L"Qxy - 指定ノートぶんの上昇ポルタメント。X = 速さ、Y = ノート",
	L"Rxy - 指定ノートぶんの下降ポルタメント。X = 速さ、Y = ノート",
	L"Axy - 音量スライド。X0 = 上昇、0Y = 下降",
	L"Sxx - ノートの切り捨て。XX = 待機フレーム数 [※ 7F 以下の時]",
	L"Sxx - 三角波チャンネルのリニアカウンタを設定。XX - 80 = 音価 [※ 80 以上の時。README 要参照]",
	L"Xxx - DPCM 再トリガー。XX = 待機フレーム数 [※ 1/60 秒の方のフレーム？]",
	L"Mxy - 遅延チャンネル音量。X = 遅延フレーム数、Y = チャンネル音量 [※ README 要参照]",
	L"Hxx - FDS モジュレーション深さ。XX = 深さ、XX≦3F",
	L"Hxx - FDS 自動モジュレーション速さ。XX - 80 = 乗数 [※ 80 以上の時。README 要参照]",
	L"I0x - FDS モジュレーション速さの上位バイトを設定、自動モジュレーションを無効化。[※ Ixy は別]",
	L"Ixy - FDS 自動モジュレーション。X = 乗数、Y + 1 = 分母 [※ X≧1 の時。I0x は別。README 要参照]",
	L"Jxx - FDS モジュレーション速さの下位バイト。",
	L"W0x - DPCM の音程。X≦F",
	L"H0y - 5B エンベロープ形状。bit: 3 = 継続、2 = アタック、1 = 交互、0 = ホールド [※ Nesdev wiki 要参照]",
	L"Hxy - 5B 自動エンベロープ。X - 8 = シフト量、Y = 形状",
	L"Ixx - 5B エンベロープ速さの上位バイト。",
	L"Jxx - 5B エンベロープ速さの下位バイト。",
	L"Wxx - 5B ノイズ音程。最低 1F から",
	L"Hxx - VRC7 カスタムパッチのポート。XX = レジスタのアドレス",
	L"Ixx - VRC7 カスタムパッチの書き込み。XX = レジスタ値",
	L"Lxx - 遅延ノートリリース。XX = 遅延フレーム数 [※ XX≦7F。README 要参照]",
	L"Oxx - グルーヴを XX に設定。",
	L"Txy - 上向き遅延トランスポーズ。X = 遅延フレーム数、Y = 半音オフセット [※ X≦7。README 要参照]",
	L"Txy - 下向き遅延トランスポーズ。X - 8 = 遅延フレーム数、Y = 半音オフセット [※ X≧8。README 要参照]",
	L"Zxx - N163 波形バッファアクセス。XX = 領域バイト [※ XX≦7E。Z7Fでリセット。README 要参照]",
	L"Exx - FDS 音量エンベロープ (アタック), XX = 速さ [※ 3F 以下の時。EE0 で無効化。README 要参照]",
	L"Exx - FDS 音量エンベロープ (ディケイ), XX - 40 = 速さ [※ 40≦XX≦7F の時。EE0 で無効化。README 要参照]",
	L"Zxx - FDS 自動モジュレーション速さのバイアス値、XX - 80 = オフセット [※ デフォルト Z80。README 要参照]",
};
