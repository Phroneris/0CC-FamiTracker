...
const LPCWSTR EFFECT_TEXTS[] = {		// // //	/// jp: 半角スペースを除去し全角記号を使用（エフェクトヘルプのみ）
	L"（未使用）",
	L"Fxx：行TickをXXに設定、グルーヴを取り消し。[※1F以下の時。20以上では別機能］",
	L"Fxx：BPMをXXに設定。[※20以上の時。1F以下では別機能］",
	L"Bxx：フレームXXの始端に飛ぶ。",
	L"Dxx：次のフレームの行XXに飛ぶ。",
	L"Cxx：楽曲を停止。",
	L"Exx：音長カウンタ番号をXXに設定。[※XX≦1F。EExとREADME要参照］",
	L"EEx：音長カウンタのモード設定。0bit目はディケイを、1bit目は音長を有効化。［※ExxとREADME要参照］",
	L"3xx：自動ポルタメント。XX＝速さ。",
	L"（未使用）",
	L"Hxy：上昇ハードウェアスウィープ。X＝速さ、Y＝幅。",
	L"Ixy：下降ハードウェアスウィープ。X＝速さ、Y＝幅。",
	L"0xy：アルペジオ。X＝2音目、Y＝3音目。［※Y＝0で2音だけになる］",
	L"4xy：ビブラート。X＝速さ、Y＝深さ。",
	L"7xy：トレモロ。X＝速さ、Y＝深さ。",
	L"Pxx：音程。XX－80＝オフセット。［※つまりP80が基準音程］",
	L"Gxx：行の遅延。XX＝フレーム数。［※3連符の作成に便利］",
	L"Zxx：DPCMデルタカウンタの設定。XX＝DCバイアス。［※XX≦7F］",
	L"1xx：音程の上昇スライド。XX＝速さ。",
	L"2xx：音程の下降スライド。XX＝速さ。",
	L"Vxx：矩形波のデューティ比やノイズモードをXXに設定。",
	L"Vxx：N163の波形番号をXXに設定。",
	L"Vxx：VRC7のパッチ番号をXXに設定。",
	L"Yxx：DPCMサンプル開始位置のオフセットをXXに設定。[※1ごとに64バイトぶん］",
	L"Qxy：指定ノートぶんの上昇ポルタメント。X＝速さ、Y＝ノート。",
	L"Rxy：指定ノートぶんの下降ポルタメント。X＝速さ、Y＝ノート。",
	L"Axy：音量スライド。X0で上昇、0Yで下降。",
	L"Sxx：ノートの切り捨て。XX＝待機フレーム数。［※XX≦7F。三角波のみ80以上で別機能］",
	L"Sxx：三角波チャンネルのリニアカウンタを設定。XX－80＝音長。［※80以上の時。240Hz。README要参照］",
	L"Xxx：DPCM再トリガー。XX＝待機フレーム数。［※1/60秒の方のフレーム？］",
	L"Mxy：遅延チャンネル音量。X＝遅延フレーム数、Y＝チャンネル音量。［※README要参照］",
	L"Hxx：FDSモジュレーション深さ。XX＝深さ、XX≦3F。",
	L"Hxx：FDS自動モジュレーション速さ。XX－80＝乗数。［※80以上の時。README要参照］",
	L"I0x：FDSモジュレーション速さの上位バイトを設定、自動モジュレーションを無効化。[※Ixyは別機能］",
	L"Ixy：FDS自動モジュレーション。X＝乗数、Y＋1＝分母。［※X≧1の時。I0xは別機能。README要参照］",
	L"Jxx：FDSモジュレーション速さの下位バイト。",
	L"W0x：DPCMの音程。X≦F。",
	L"H0y：5Bエンベロープ形状。各bitは3＝継続、2＝アタック、1＝交互、0＝ホールド。［※何らかの資料を要参照］",
	L"Hxy：5B自動エンベロープ。X－8＝シフト量、Y＝形状。",
	L"Ixx：5Bエンベロープ速さの上位バイト。",
	L"Jxx：5Bエンベロープ速さの下位バイト。",
	L"Wxx：5Bノイズ音程。1Fが最低。",
	L"Hxx：VRC7カスタムパッチのポート。XX＝レジスタのアドレス。",
	L"Ixx：VRC7カスタムパッチの書き込み。XX＝レジスタ値。",
	L"Lxx：遅延ノートリリース。XX＝遅延フレーム数。［※XX≦7F。README要参照］",
	L"Oxx：グルーヴをXXに設定。",
	L"Txy：上向き遅延トランスポーズ。X＝遅延フレーム数、Y＝半音オフセット。［※X≦7。README要参照］",
	L"Txy：下向き遅延トランスポーズ。X－8＝遅延フレーム数、Y＝半音オフセット。［※X≧8。README要参照］",
	L"Zxx：N163波形バッファアクセス。XX＝領域バイト。［※XX≦7E。Z7Fでリセット。README要参照］",
	L"Exx：FDS音量エンベロープ (アタック)。XX＝速さ。［※3F以下の時。EE0で無効化。README要参照］",
	L"Exx：FDS音量エンベロープ (ディケイ)。XX－40＝速さ。［※40≦XX≦7Fの時。EE0で無効化。README要参照］",
	L"Zxx：FDS自動モジュレーション速さのバイアス値。XX－80＝オフセット。［※デフォルトはZ80。README要参照］",
};
